                ...
                <div class="dashboard-main">
                    <div class="dashboard-content-section"
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-5">
                            ...
                            <a href="{{ route('student.notifications.readAll') }}" class="theme-btn"
                        ...
                        <div class="notif-list-container">
                            @forelse($notifications as $notification)
                                @php
                                    ...
                                    // Minimal design logic based on screenshot
                                    $bgColor = 'rgba(0, 0, 0, 0.02)';
                                    $accentColor = 'var(--theme)';
                                    $iconClass = 'fa-bell';

                                    if ($isUnread) {
                                        $bgColor = '#fffaf3'; // Light beige/orange for unread
                                                ...
                                                <div class="d-flex align-items-center gap-2 mb-2">
                                                    ...
                                                    @if($isUnread)
                                                        <span class="badge bg-theme-light text-theme ms-2"
                                                            style="font-size: 10px; padding: 4px 8px; font-weight: 700; text-transform: uppercase;">{{ __('student.new') }}</span>
                                                    @endif
                                                </div>
                                        ...
                                        <div class="text-end d-flex flex-column justify-content-between align-items-end"
                                            ...
                                            @if($isUnread)
                                                <form action="{{ route('student.notifications.read', $notification->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-link text-theme p-0 text-decoration-none"
                                                        style="font-size: 13px; font-weight: 600;">
                                                        {{ __('student.mark_as_read') }}
                                                    </button>
                                                </form>
                                            @else
                                                <span>&nbsp;</span>
                            ...
                            @empty
                                <div class="text-center py-5">
                                    ...
                                    <h4 class="text-dark fw-bold">{{ __('student.all_caught_up') }}</h4>
                                    <p class="text-muted">{{ __('student.no_notifications_desc') }}</p>
                                </div>
                            @endforelse
                        </div>
